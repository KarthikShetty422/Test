...
var startBooking;
    ...
    function popUpVal(popupdata) {
        ...
        // Change the cursor to a pointer when the mouse is over the places layer.
            map.on('mouseenter', popupdata , function (e) {
                setTimeout( () =>{
                    var inputFields = document.getElementsByClassName('myInput');
                    startbooking(inputFields);
              },3000);
                 map.getCanvas().style.cursor = 'pointer';
                 var originairport = inputvalarray[0];
                 var destinationairport = inputvalarray[1];
                 originairport = originairport.split("(");
                 originairport = originairport[1];
                 originairport = originairport.split("-");
                 var originairportCode = originairport[0];
                 var originairportName = originairport[1];
                 ...
                 destinationairport = destinationairport.split("-");
                 var destinationairportCode = destinationairport[0];
                 var destinationairportName = destinationairport[1];
                 ...
                 '</br>'+'<button type="button" target="_blank"  class="btn btn-primary start-booking input-container"  item-index="1">'+'StartBooking'+'</button>'
    ...
    function displayRoutes(map, element, id) {
        ...
        inputvalarray.push(destination);
        inputTextVal = inputTextVal.split("(");
        inputTextVal = inputTextVal[1];
        inputTextVal = inputTextVal.split("-");
        inputTextVal = inputTextVal[0];
            ...
            success: function(response) {
                ...
                routes.forEach(function (segment) {
                    segment.segments.forEach(function (segmentdata) {
                        ...
                        airportsData.push(destinationAirportdata);
